...
| 文件 | 给谁用 | 内容 |
|------|--------|------|
| **design.md** | 人 + AI 会话 | 品牌 SSOT，六段式：命名调性 / 版式语法（构图骨架 · 明度结构 · 密度）/ 配色 token 表（hex + 用途 + 配比，含可选暗色变体）/ 字体配对 / 视觉语言元件 / Do & Don't 红线 |
...
→ ④ 细节轮循环(浏览器点选)
       ↑               │ 不满意区块重生成
       └───────────────┘ 满意区块锁定
→ ⑤ 收尾(暗色变体 + 调性回放 + 命名 + Do/Don't) → ⑥ design.md + theme.css
```
...
**核心机制是 picker**：候选方案不是色块表——hex 码肉眼读不出感觉——而是渲染成实景小样（迷你封面）的网页。第一轮是**方向板轮**：3-4 个构图骨架、明度结构都不同的完整方向板，整板单选——先定"隔三米能认出你"的骨，再进细节轮逐区块打磨色彩字体质感。你在浏览器里：
...
提交后 Claude 读取你的选择：满意的区块**锁定**不再出现，不满意的按反馈重做，循环三五轮收敛。视觉判断走眼睛，语言判断（命名、调性、红线）走对话——但对话同样只出选择题：调性从你的点选行为里提炼后**回放**给你纠正，你不需要会表达，只需要会说"不对"。
...
原型只是第一轮候选的起点，循环微调后才是个人化结果。每个原型自带**版式签名**（构图骨架 + 明度结构 + 密度），是方向板轮的骨；细节轮的色板/字体/质感/元件/形状五个区块是**独立轴**，内置《混搭规则》强制候选跨原型取材（"蓝图的元件 + 粉彩的色板"），并附配方示例与冲突警告——个性来自组合，不来自单选。
...
- `references/design-md-template.md` — design.md 六段式模板 + theme.css 翻译骨架 + 硬编码自检清单
