...
| **方向板轮**（第 1 轮） | 10 个完整方向板（硬性，全部为心动原型要素的融合重组），候选**自带模板**（`html` + `css`），结构级差异 | 整板单选 |
...
**微差异区块必须配放大样片**：字体配对、元件这类候选间差异细微的区块，多场景共享模板缩到缩略图尺寸后差异不可辨——候选加 `thumbHtml`（仅缩略图替换模板，主预览不受影响，css 放 preview.css 并命名空间）：字体候选给大字样、元件候选给放大的单元件特写。色板候选不需要（底色一变即可辨）。

字体大字样的设计要领：**放大"会变的角色"**——候选间变的是西文标题字体就把西文放大（如 72px "Rag 12"，中文做小字对照行），变的是中文字体就放大中文。西文字体不含中文字形、中文会回落到共同的 fallback——把中文放大等于让所有候选长一个样。

字体候选的特殊处理：字体没法用一个 CSS 变量切换 `@import`，把所有候选字体都在 `preview.css` 里 `@import` 进来，候选 `vars` 切 `--font-title` / `--font-body` / `--font-mono` 的值。

**色板候选必须带满角色**：每个色板候选的 `vars` 须覆盖 design.md 配色表的全部角色（底色/次级底/主文字/结构色/点缀色，按需加徽章色/次级灰，共 5-8 个）——只带 4 个核心色的候选是规格违例，缺的角色到产出时已无人决策，只能靠模型脑补。预览模板相应地要让次级底/次级灰有出场位置（如卡片底、注释字）。
...
- **板数硬性 10 板**：全部为心动票原型设计要素的融合与重组；碰撞板双方必须都是心动原型（与无感/未投票原型的组合只能记作试探板名额）；配比细则见 SKILL.md ③《生成纪律》
- **禁止速筛复读**：任何方向板不得与单一原型同构同样式——心动原型必须经过结构碰撞（与其他心动原型组合）或定向变体（按事实题画像改造）才能上板
- **整组自检（防集合级复读）**：出板前把 10 板并排自查——若多数板可一眼指认唯一主导原型、且主导原型集合与心动票一一对应，整组视同速筛复读，必须重组；单板能引用组合公式不构成豁免
- 排除票原型的结构与标志元件不得上板
...
- **语法锚定**：方向板只是语法的可视化载体。锁定后写进 design.md 版式语法段的是**跨场景不变量**（对齐纪律/明度结构/密度档/签名手法）+ 按画幅的应用指引（见 `design-md-template.md`）。坐标禁令分层：**全局规则禁止具体坐标**（"信息入口从左上进"是语法，"标题在左上角 x,y"是排版，排版是消费方引擎的事）；**封面/题图构图层允许具体构图**（"元件固定右下角"——封面是品牌橱窗，其构图本身是品牌资产）
...
零素材探索模式必出；素材提取/参考借鉴模式跳过投票区块（素材本身就是气质信号），事实题并入方向板轮页顶。

**速筛卡是预制标准件**（`assets/picker/archetype-cards.js`，随 skill 分发）——心理测验的题目必须每人相同：16 张卡标题文案统一、卡间只差风格，现场生成导致的渲染差异不会污染票果。**不要现场重写卡片**；改卡先改原型库再改预制文件。

生成第 0 轮 picker-data.js：把预制文件内容原样拼在最前，随后 PICKER_DATA 引用它暴露的两个全局量：

```bash
cat <skill目录>/assets/picker/archetype-cards.js > design-extract-picker/picker-data.js
# 再向同一文件追加 window.PICKER_DATA = {...}
```
...
window.PICKER_DATA = {
  title: "第 0 轮 · 气质速筛",
  round: 0,
  // @import 须位于样式首部——预制字体引入恰好放 preview.css
  preview: { html: "", css: window.ARCHETYPE_FONTS_CSS, baseVars: {}, hidden: true },
  sections: [
    ...
    { id: "vibe", type: "vote", title: "凭直觉投票",
      ...
      candidates: window.ARCHETYPE_CARDS }   // 16 张预制卡，可按《映射建议》重排序
  ]
};
```
...
- **全量呈现，不预筛**：16 个原型全部出卡——速筛的意义就是把"模型猜你喜欢什么"换成"你自己看见了再说"。原型库《映射建议》只决定卡片排序（领域匹配的排前面，`window.ARCHETYPE_CARDS` 数组可重排），不决定谁被隐藏
...
- **边界票型**：零心动票 → 不许硬出方向板，回对话兜底（见 SKILL.md《偏好画像》）；心动票维度矛盾（交集为空）→ 按维度分簇出板 + 试探板求证哪一极，不许取平均
...
调性/命名选定后如有口头修正（feedback），按修正改写再产出，不再加轮。**修正必须沉淀进 design.md**：改的是调性 → 命名调性段；改的是 wordmark 呈现（如"英文再小一点"）→ 视觉语言段写成可执行规则（如"wordmark 副标 ≤ 主标 1/4 字高"）——只改当轮渲染、不落文件等于丢失。
...
| 对话里要求推翻已锁区块 | 解锁该区块按理由重出；只连带直接受约束的未锁区块；推翻理由入红线池 + 更新画像（处理细则见 SKILL.md ④） |
...
- 候选数量每区块 2-4 个，多了选择瘫痪；三个例外：方向板轮（硬性 10 板——结构组合空间必须展开，整板大样一眼可比，见 SKILL.md ③）、色板与字体配对（6-10 个，按画像匹配度排序——组合空间大、缩略图一眼可比）、速筛轮 vote 区块（16 张全量——投票单卡成本低，不致瘫痪）
